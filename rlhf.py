...
# Load the Qwen2.5-1.5B-Instruct model onto GPU 0 for the training workload.
...
# Create a placement group that reserves GPU 1 for the vLLM inference engine.
...
handle = llm.collective_rpc.remote(
    "init_weight_update_group", args=(master_address, master_port, 1, 2)
)

model_update_group = stateless_init_process_group(
    master_address, master_port, rank=0, world_size=2, device=torch.device("cuda:0")
)
ray.get(handle)
...
# assert all(ray.get(llm.collective_rpc.remote("check_weights_changed", args=(train_model,))))
